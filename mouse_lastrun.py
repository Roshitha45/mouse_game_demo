...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "profilepic" ---
    drawing_area = visual.Rect(
        win=win, name='drawing_area',
        width=(0.5, 0.5)[0], height=(0.5, 0.5)[1],
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=3.0,
        colorSpace='rgb', lineColor=[-0.6706, -0.7490, -0.7490], fillColor='white',
        opacity=None, depth=0.0, interpolate=True)
    brush = visual.Brush(win=win, name='brush',
       lineWidth=10.0,
       lineColor=[0.0824, -0.6627, 0.7725],
       lineColorSpace='rgb',
       opacity=0.8,
       buttonRequired=True,
       depth=-1
    ...
    instructions_mouse_2 = visual.ImageStim(
        win=win,
        name='instructions_mouse_2', 
    ...
    # --- Prepare to start Routine "profilepic" ---
    ...
    profilepic.status = STARTED
    thisExp.addData('profilepic.started', profilepic.tStart)
    profilepic.maxDuration = None
    # keep track of which components have finished
    profilepicComponents = profilepic.components
    for thisComponent in profilepic.components:
    ...
    while continueRoutine and routineTimer.getTime() < 200.0:
        ...
        # *drawing_area* updates
        
        # if drawing_area is starting this frame...
        if drawing_area.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            drawing_area.frameNStart = frameN  # exact frame index
            drawing_area.tStart = t  # local t and not account for scr refresh
            drawing_area.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(drawing_area, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'drawing_area.started')
            # update status
            drawing_area.status = STARTED
            drawing_area.setAutoDraw(True)
        
        # if drawing_area is active this frame...
        if drawing_area.status == STARTED:
            # update params
            pass
        
        # if drawing_area is stopping this frame...
        if drawing_area.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > drawing_area.tStartRefresh + 200-frameTolerance:
                # keep track of stop time/frame for later
                drawing_area.tStop = t  # not accounting for scr refresh
                drawing_area.tStopRefresh = tThisFlipGlobal  # on global time
                drawing_area.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'drawing_area.stopped')
                # update status
                drawing_area.status = FINISHED
                drawing_area.setAutoDraw(False)
        ...
        if brush.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'brush.started')
            # update status
        ...
        if brush.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > brush.tStartRefresh + 200-frameTolerance:
                # keep track of stop time/frame for later
                brush.tStop = t  # not accounting for scr refresh
                brush.tStopRefresh = tThisFlipGlobal  # on global time
                brush.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'brush.stopped')
                # update status
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            profilepic.forceEnded = routineForceEnded = True
            break
    ...
    # --- Ending Routine "profilepic" ---
    for thisComponent in profilepic.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for profilepic
    ...
    # create an object to store info about Routine instructions_post
    instructions_post = data.Routine(
        name='instructions_post',
        components=[instructions_mouse_2],
    )
    instructions_post.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for instructions_post
    instructions_post.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instructions_post.tStart = globalClock.getTime(format='float')
    instructions_post.status = STARTED
    thisExp.addData('instructions_post.started', instructions_post.tStart)
    instructions_post.maxDuration = None
    # keep track of which components have finished
    instructions_postComponents = instructions_post.components
    for thisComponent in instructions_post.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instructions_post" ---
    instructions_post.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 1.5:
        ...
        # if instructions_mouse_2 is starting this frame...
        if instructions_mouse_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instructions_mouse_2.frameNStart = frameN  # exact frame index
            instructions_mouse_2.tStart = t  # local t and not account for scr refresh
            instructions_mouse_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instructions_mouse_2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instructions_mouse_2.started')
            # update status
            instructions_mouse_2.status = STARTED
            instructions_mouse_2.setAutoDraw(True)
        
        # if instructions_mouse_2 is active this frame...
        if instructions_mouse_2.status == STARTED:
            # update params
            pass
        
        # if instructions_mouse_2 is stopping this frame...
        if instructions_mouse_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instructions_mouse_2.tStartRefresh + 1.5-frameTolerance:
                # keep track of stop time/frame for later
                instructions_mouse_2.tStop = t  # not accounting for scr refresh
                instructions_mouse_2.tStopRefresh = tThisFlipGlobal  # on global time
                instructions_mouse_2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instructions_mouse_2.stopped')
                # update status
                instructions_mouse_2.status = FINISHED
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=instructions_post,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instructions_post.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instructions_post.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in instructions_post.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instructions_post
    instructions_post.tStop = globalClock.getTime(format='float')
    instructions_post.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instructions_post.stopped', instructions_post.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if instructions_post.maxDurationReached:
        routineTimer.addTime(-instructions_post.maxDuration)
    elif instructions_post.forceEnded:
        routineTimer.reset()
    else:
